...
for (i in web_list) {
    if (web_list[i].title == '捏脸') {
        web_list_html += `<div id="web_8_30" class="col-md-4 web-grid web_8 is_star" data-toggle="modal" data-target="#exampleModal"><span style="cursor:pointer"><div  data-html="true" class="services-inner-box web-single clearfix" ontouchstart=""><span class="star iconfont">&#xe639;</span><h2>我的在线书签</h2><p><span class="web-kind" style="margin-right:0">实用</span></p></div></span></div>`;
    }
    let like_num_id = `${web_list[i].kind}_${web_list[i].id}`;
    if ($(window).width() >= 768) {
        web_list_html += `<div id="${like_num_id}" class="col-md-4 web-grid web-grid-web ${web_list[i].kind} is_${web_list[i].star}"><mya href="${web_list[i].href}"><div data-toggle="popover" data-html="true" data-content="${web_list[i].slogan}" class="services-inner-box web-single clearfix" ontouchstart=""><span class="${web_list[i].star} iconfont">&#xe639;</span><h2>${web_list[i].title}</h2><p><span class="web-kind">${web_list[i].kind_name}</span></p></div></mya></div>`;
    } else {
...
$('.web-grid-phone').click(function () {
    let like_num = $(this).find('p>.like-num').text();
    ...
    $('.phone-modal').find('.like-num').text(like_num);
    $('.phone-modal').find('.iconfont').removeClass('like_flag');
    $('.phone-modal').find('.like-num').removeClass('like_flag');
